/*jslint browser: true, es5: true, evil: true */
...
(function ($) {
    ...
    */
    ...
    /*
     Returns a object of all data attributes beginning with an @ and their values of a given dom element
     {@name: value1, @other: value2}
     */
    function getAttributes(e) {
        /* 
         returns all HTML5 custom data attributes as an object:
         <div id='prod' data-id='10' data-cat='toy' data-cid='42'>blah</div> -> { "id":10, "cat":"toy", "cid":42 }
         jQuery (>=1.4.4)
         */
        var dataDict = e.data();
        
        //Filter data attributes for @ character
        $.each(dataDict, function (key, value) {
            if (key.indexOf('@') !== 0) {
                delete dataDict[key];
            }
        });
    ...
    function resolveReference(childAttr) {
        ...
        $.each(childAttr, function (i, arry) {
            ...
            $.each(attr, function (k, v) {
                ...
                if (typeof (v) !== 'object') {
                    console.error(attr);
                    throw "Format has to be: data-@name={event, handler} or {id, attribute} for a reference'";
                }
            ...
            $.each(domAt[1], function (key, value) {
                ...
                if (typeof (value.event) === 'object') {
                    $.each(value.event, function (i, val) {
                        if (val === event) {
                            handleEvent(domAt[0], key.substring(1), value.event, value.handler, eventValue);
                        }
                    });
                } else {
